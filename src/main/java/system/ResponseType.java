...
public abstract class ResponseType extends Middleware {
	...
	protected final boolean handle() {
		...
		prefixes.add(RuntimeVariables.getInstance().getCommandPrefix().toLowerCase());
		if(!StringUtils.isNullOrWhiteSpace(this.config.customPrefix)){
			prefixes.add(this.config.customPrefix);
		}
			...
			if (this.getMessage().getContent().toLowerCase().startsWith(PREFIX)) {
				...
				if (this.getMessage().getContent().contains(" ")) {
					...
					// Prefix seperated with " "
						...
						// Line above will replace nothing, if args are empty. Clear args in that case
						if(this.argumentSection.equals(this.getMessage().getContent())){
							this.argumentSection = "";
						}
				...
				break; // Do NOT continue with next prefix
			}
